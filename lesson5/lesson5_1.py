...
class Window(ThemedTk):
    def __init__(self, *args, **kwargs):
        ...
        ttk.Label(bottomFrame, text='UserName:').grid(
            column=0, row=0, padx=(10, 0), sticky='E')

        self.username = tk.StringVar()
        ttk.Entry(bottomFrame, textvariable=self.username).grid(
            column=1, row=0, pady=10)

        ttk.Label(bottomFrame, text='Password:').grid(
            row=1, column=0, sticky='E')

        self.password = tk.StringVar()
        ttk.Entry(bottomFrame, textvariable=self.password,show='*').grid(
            column=1, row=1, pady=10, padx=10)
