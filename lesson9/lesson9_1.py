...
class Window(ThemedTk):
    def __init__(self, *args, **kwargs):
        ...
        ttk.Label(topFrame, text='空氣品質指標(AQI)(歷史資料)',
        ...
        sitenames_cb = ttk.Combobox(
            self.selectedFrame, textvariable=self.selected_county, values=counties, state='readonly')
    ...
    def item_selected(self, event):
        for selected_item in self.tree.selection():
            ...
            dialog = view.MyCustomDialog(parent=self, title=f'{
                                         record["values"][1]}-{record["values"][2]}', record=record['values'])
